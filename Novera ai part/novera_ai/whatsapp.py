...
returned instead of sent — so the whole flow is testable without a live number.
"""
...
def send_message(body: str, to: Optional[str] = None, simulate: bool = False) -> dict[str, Any]:
    ...
    except Exception as exc:  # keep the flow alive even if the Graph API errors
...
def offer_message(case_id: str | None = None) -> str:
