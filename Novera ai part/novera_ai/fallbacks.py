"""Deterministic bilingual fallbacks used when the LLM is unavailable."""
    ...
    areas = [
        {
            ...
            "status": status,
            "note": (
                f"{names[aid]}: المؤشرات {sw[status]} في هذه القراءة."
                if lang == "ar"
...
def self_care(reading: dict[str, Any], lang: str) -> dict[str, Any]:
    ...
    area_tips = [
        {
            ...
            "status": status,
            "tip": (
                f"{names[aid]}: المؤشرات {sw[status]}. حافظ على عادات متوازنة وتابع في القراءة القادمة."
                if lang == "ar"
    ...
    return {
        ...
        "focusBody": (
            "خطة عامة للعناية الذاتية مبنية على قراءتك الأخيرة. أضف سياق طبيبك في المحادثة لتخصيصها أكثر."
            if lang == "ar"
...
def chat(messages: list[dict[str, Any]], ctx: dict[str, Any], lang: str) -> dict[str, Any]:
    ...
    return {
        "reply": (
            "شكراً لمشاركتك هذا. سجّلت ملاحظاتك وسأضعها في اعتباري عند تحديث خطة العناية الذاتية. (وضع بديل بدون اتصال بالذكاء الاصطناعي.)"
            if lang == "ar"
