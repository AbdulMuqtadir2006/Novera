...
def _delayed_reading_followup(owner: dict) -> None:
    """A separate, later "how are you feeling" wellness check-in — NOT the
    screening run, which still fires immediately (see add_reading below).
    ...
    SECONDS. Not persisted: an app restart mid-delay silently drops the
    pending follow-up, same as every other in-process background task in
    ...
    whatsapp_agent.handle_trigger("reading.followup", owner)
...
def add_reading(body: ReadingIn, background_tasks: BackgroundTasks):
    ...
    if config.AUTO_AGENT_ENABLED and owner_user_id is not None:
        ...
        if owner:
            ...
            # Separate, later "how are you feeling" check-in (2026-08-22,
            # Hassan's call) — deliberately independent of the screening run
            # above, not a delayed version of it. See _delayed_reading_followup.
            background_tasks.add_task(_delayed_reading_followup, owner)
