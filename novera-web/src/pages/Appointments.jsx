...
import {
  ...
  Workflow, ShieldAlert, ShieldCheck,
} from "lucide-react";
import { PageShell } from "../components/layout/PageShell";
import {
  sendAppointmentOffer, replyAppointment, getBookings, getClinic, predictOrgan, runPipeline,
} from "../lib/api";
...
function PipelinePanel({ t }) {
  ...
  const crossed = result && !result.error && result.threshold_crossed;

  return (
    <div className="light-card p-6">
      <h3 className="flex items-center gap-2 font-display text-lg font-bold text-depth">
        <Workflow size={18} className="text-signal" /> {t("appt.pipelineTitle")}
      </h3>
      <p className="mt-1 text-sm text-depth/65">{t("appt.pipelineHint")}</p>
      <button type="button" onClick={run} disabled={loading} className="btn-primary mt-4 disabled:opacity-60">
        {loading ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
        {loading ? t("appt.running") : t("appt.runPipeline")}
      </button>
      ...
      {result && !result.error && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5 space-y-4">
          <div
            ...
            style={{
              backgroundColor: crossed ? "rgba(214,69,69,0.1)" : "rgba(34,160,107,0.1)",
              color: crossed ? "#d64545" : "#1f8a5b",
            }}
          >
            {crossed ? <ShieldAlert size={18} /> : <ShieldCheck size={18} />}
            {crossed ? t("appt.thresholdCrossed") : t("appt.allClear")}
          </div>

          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl bg-mist/60 px-3 py-2">
              <p className="font-mono text-[11px] uppercase tracking-wide text-depth/45">{t("appt.qaConfidence")}</p>
              <p className="mt-0.5 font-mono text-lg font-semibold text-depth">
                {Math.round((result.confidence_score || 0) * 100)}%
              </p>
            </div>
            <div className="rounded-xl bg-mist/60 px-3 py-2">
              <p className="font-mono text-[11px] uppercase tracking-wide text-depth/45">{t("appt.flaggedAreas")}</p>
              <p className="mt-0.5 text-sm font-medium text-depth">
                {result.flagged_domains?.length ? result.flagged_domains.length : "—"}
              </p>
            </div>
          </div>
          ...
          {result.flagged_domains?.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {result.flagged_domains.map((d) => (
                <span key={d} className="rounded-full bg-vital/12 px-2.5 py-1 text-xs font-medium text-depth/80">{d}</span>
              ))}
            </div>
          )}

          <div>
            <p className="mb-2 font-mono text-[11px] uppercase tracking-wide text-depth/45">{t("appt.agentPath")}</p>
            <div className="flex flex-wrap items-center gap-1.5">
              {(result.trace || []).map((node, i) => (
                <span key={i} className="inline-flex items-center gap-1.5">
                  <span className="rounded-md border border-signal/20 bg-signal/8 px-2 py-1 font-mono text-[11px] text-depth/75">{node}</span>
                  {i < result.trace.length - 1 && <span className="text-depth/30">›</span>}
                </span>
              ))}
            </div>
          </div>
